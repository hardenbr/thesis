...
The Standard Model (SM) is the theoretical framework that best describes the
experimentally observed phenomena of the fundamental particles and their interactions.
In 2012, the last piece to the SM was put into place with the discovery of the Higgs boson.
This discovery, the theory for which is described in Sections~\ref{sec:SM}
and \ref{sec:discovery},
...
Despite its many successes, the theory
is not perfect, and it remains an overarching theme of particle physics to unify physical processes
at all energy scales under one single framework. Some of these shortcomings are described in
Section~\ref{sec:SMshortcomings}, giving a little taste for what could be discovered
with the accumulation of more data at higher center-of-mass energies by the big experiments
...
\begin{equation}
V\left(\phi^\dagger\phi\right) = -\mu\phi^\dagger\phi + \lambda\left(\phi^\dagger\phi\right)^2 \,,
\end{equation}
...
\begin{figure}[htbp!]
 \begin{center}
    \includegraphics[width=0.75\textwidth]{figures/intro/crosssections2012_v5.pdf}
      \end{center}
...
\begin{table}[htbp!]
  ...
  \input{tables/SMHiggsBR}
  \label{table:BR_SMhiggs}
\end{table}

\begin{figure}[htbp!]
 \begin{center}
    \includegraphics[width=0.6\textwidth]{figures/intro/sqr_mlz_ccc_mH125_prod.pdf}
      \end{center}
...
\begin{figure}[htbp!]
 \begin{center}
    \includegraphics[width=0.6\textwidth]{figures/intro/sqr_mlz_ccc_mH125_decay.pdf}
      \end{center}
...
\begin{figure}[htbp!]
 \begin{center}
    \includegraphics[width=0.6\textwidth]{figures/intro/sqr_mass_all.pdf}
      \end{center}
...
for example, matter-antimatter asymmetry, hierarchy, naturalness, and the generations of fermions.
...
Ordinary matter, as built-up from the SM, only accounts for 4.9\% of the
mass-energy in the known universe~\cite{Clowe:2006eq,Ade:2013sjv}. Most of the mass-energy is made
up of dark matter at 26.8\% and dark energy at 68.3\%, the nature of which is largely unknown.

The presence of dark matter is inferred from observing the way ordinary matter behaves at
...
universe. These measurements indicate that the rate of expansion is increasing. Two candidates
to account for the source of the accelerating expansion are a cosmological constant or 
scalar fields. The former gives a constant energy density in space and time, while the latter gives
rise to an energy density that varies (perhaps very slowly) in space and time.
...
is predicted to be 9.96 fb, including next-to-next-to-leading order (NNLO) corrections.
...
- \frac{\sqrt{2} m_t}{v}\left(v+  \kapt H  +  \frac{ \sqrt{2} c_2}{v} H  H \right) \left( \bar{t}_L t_R + h.c. \right)  \,,
\end{split}
\label{eq:Hpot}
\end{equation}
...
\label{eq:nonrescx2}
\end{equation}
where $\bar{\sigma}_\text{NNLO} = 97.12 \mbox{ fb}$, $\alpha = 0.475$, $\beta = 0.185$, $A_1 = -1.89$,
...
%  \input{tables/dihiggsxsec}
%  \label{table:ggHH_SM_XS}
%\end{table}
%\url{https://indico.cern.ch/event/353146/contribution/13/material/slides/0.pdf}, still unpublished.

The results are interpreted in a scenario where only dimension-six operators involving one Higgs doublet
...
-  c_t \left( 1+ \frac{c_t}{\Lambda^2} \phi^{\dagger}\phi\right) \left(\bar{Q}_L \phi^{\dagger}t_R + h.c. \right)  \,.
\end{split}
\label{eq:Hpot6}
\end{equation}
...
\end{equation}
\begin{equation}
c_2  =  \frac{v^2}{\sqrt{2}\Lambda^2} \left( c_t -\frac{1}{2} c_H \right) \,.
\end{equation}
\end{subequations}
The bare top quark mass and Higgs mass are
\begin{subequations}
\begin{equation}
m_t = y_{t}\, v \left(1+c_t \, \frac{v^2}{\Lambda^2}\right)
\end{equation}
\begin{equation}
m_H^2  = 2 \lambda_6\, v^2 \left(1+c_6 \frac{v^2}{\Lambda^2}\right) \,.
\end{equation}
\end{subequations}
