...
actually sloped at 1.5 degrees toward Lac Léman, causing the tunnel depth to vary between 50 and 150 m.
...
is provided, starting from the closest to the main interaction point (IP) and proceeding
...
allows for the collection of this free electron and electron hole. If the charged collected during
...
The ECAL~\cite{ecaltdr} is a calorimeter that focuses on measuring the energy of electromagnetic showers.
...
layer of absorption for any hadrons that make it through preceding layers.

A final component covers pseudorapidities $3.0 < |\eta|< 5.0$. In this region, the damage from
radiation is especially high, so here a steel and quartz-fibre \v{C}erenkov calorimeter is used.
...
and most of HCAL with respect to each other and the IP. Missing from the HCAL are the 
